...
When editing or adding pages, **commit to this aesthetic** — chunky borders + hard shadows + sticker chips + bold display type. Do **not** drift toward generic AI design (Inter, soft purple gradients, rounded SaaS cards with subtle shadows). The local skill at [.Codex/skills/frontend-design/SKILL.md](.Codex/skills/frontend-design/SKILL.md) — copied in from the global `frontend-design` plugin — captures this principle and auto-loads in future sessions.
